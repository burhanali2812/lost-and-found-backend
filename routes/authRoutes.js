  ...
  async (req, res) => {
    try {
      const newImage = new Image({
        imageUrl: `https://lost-and-found-backend-xi.vercel.app/uploads/${req.file.filename}`,
      });
      await newImage.save();
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!newResponse.ok) {
        return res
          .status(500)
          .json({ success: false, message: "Notification failed to send" });
      }
      ...
      res
        ...
        .json({
          success: true,
          message: "Account created successfully and notification sent",
          user,
        });
    } catch (error) {
...
router.post(
  ...
  async (req, res) => {
    ...
    const imageUrls = req.files.map(
      (file) =>
        `https://lost-and-found-backend-xi.vercel.app/uploads/${file.filename}`
    );
